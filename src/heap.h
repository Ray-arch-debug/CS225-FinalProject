/**
 * @file heap.h
 * Definition of a heap class. (Written in lab_heap)
 */
...
class heap
{
  public:
     ...
     */
    heap(const std::vector<std::pair<int, double>>& elems);
     ...
     */
    std::pair<int, double> peek() const;
     ...
     */
    void push(const std::pair<int, double>& elem);
     ...
     */
    void updateElem(const size_t & idx, const std::pair<int, double>& elem);
    ...
    void getElems(std::vector<std::pair<int, double>> & heaped) const;
    ...
    */
     ...
     */
    std::vector<std::pair<int, double>> _elems;
    ...
    */
    ...
    /*
        map from csv index to heap array index
    */
    std::map<int, int> positions_;

    int size() const;

    void updateDistance(int csvIdx, double newDistance);

    bool isInHeap(int csvIdx);

    const int BigNum = INT32_MAX;
};